...
import { Language } from '@/store/modules/app/helper'
...
const languages: Array<languageItem> = [
  { name: '简体中文', online: true, language: 'zh-CN' },
  { name: '繁體中文', online: false, language: 'zh-TW' },
  { name: 'English', online: true, language: 'en-US' },
]


const selected = computed({
  get: () =>
    languages.find((element) => element.language == appStore.language) ||
    languages[0],
  set: (val) => {
    if (!val) {
      return
    }
    appStore.setLanguage(val.language)
    ...
    <div class="relative mt-1">
      <ListboxButton
        ...
        <span class="flex items-center">
          <span :aria-label="selected.online ? 'Online' : 'Offline'" :class="[
      ...
      <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100"
        ...
        <ListboxOptions
          ...
          <ListboxOption as="template" v-for="item in languages" :key="item.name" :value="item">
            <li :class="[
              selected == item ? 'text-white bg-indigo-600' : 'text-gray-900',
